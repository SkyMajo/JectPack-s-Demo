...
public class AppBottomBar extends BottomNavigationView {
    ...
    public AppBottomBar(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        config = AppConfig.INSTANCE.getBottomBarConfig();

        int[][] state = new int[2][];
        state[0] = new int[]{android.R.attr.state_selected};
        state[1] = new int[]{};
        int[] colors = new int[]{Color.parseColor(
                (config == null ||config.activeColor==null)?"#8A2BE2":config.activeColor),
                Color.parseColor((config == null ||config.inActiveColor == null)?"#8B8989":config.inActiveColor)};
    ...
    private int getItemId(String pageUrl) {
        Destination destination = AppConfig.INSTANCE.getDestinationConfig().get(pageUrl);
        if (destination == null)
            return -1;
